        ...
        DT::datatable({
          isolate({
            ...
            rlt <- data.table(id = factor(rlt[,2], levels =  rlt[,2]),
                              ...
                              Cancer10y = risk.lfs[,2],
                              Cancer15y = risk.lfs[,3])
            colnames(rlt) <- c("ID", "Info", "ProbLFSPRO", "LFSPRO-carrier", "Chompret criteria", "Classic criteria",
                               "Cancer Type", "5 Years", "10 Years", "15 Years")
            LFSPRO.rlt <<- rlt
            rlt <- cbind('Details' = '&oplus;', rlt)
            rownames(rlt) <- as.character(rlt$ID)
            rlt
          })
        }, container = sketch, filter = 'top', escape = F,
        options = list(
          columnDefs = list(
